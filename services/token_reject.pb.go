...
func (x *TokenRejectTransactionBody) GetOwner() *common.AccountID {
	if x != nil {
		return x.Owner
	}
	return nil
...
func (x *TokenReference) GetFungibleToken() *common.TokenID {
	if x != nil {
...
func (x *TokenReference) GetNft() *common.NftID {
	if x != nil {
...
type TokenReference_FungibleToken struct {
	...
	FungibleToken *common.TokenID `protobuf:"bytes,1,opt,name=fungible_token,json=fungibleToken,proto3,oneof"`
}

type TokenReference_Nft struct {
	// *
	// A single specific serialized non-fungible/unique token.
	Nft *common.NftID `protobuf:"bytes,2,opt,name=nft,proto3,oneof"`
}
