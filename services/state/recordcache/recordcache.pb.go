// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        v5.29.3
// source: state/recordcache/recordcache.proto

package recordcache

import (
	common "github.com/cordialsys/hedera-protobufs-go/common"
	services "github.com/cordialsys/hedera-protobufs-go/services"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// As transactions are handled and records and receipts are created, they are stored in state for a configured time
// limit (perhaps, for example, 3 minutes). During this time window, any client can query the node and get the record
// or receipt for the transaction. The TransactionRecordEntry is the object stored in state with this information.
type TransactionRecordEntry struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// *
	// The ID of the node that submitted the transaction to consensus. The ID is the ID of the node as known by the
	// address book. Valid node IDs are in the range 0..2^63-1, inclusive.
	NodeId int64 `protobuf:"varint,1,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"`
	// *
	// The AccountID of the payer of the transaction. This may be the same as the account ID within the Transaction ID
	// of the record, or it may be the account ID of the node that submitted the transaction to consensus if the account
	// ID in the Transaction ID is not able to pay.
	PayerAccountId *common.AccountID `protobuf:"bytes,2,opt,name=payer_account_id,json=payerAccountId,proto3" json:"payer_account_id,omitempty"`
	// *
	// The transaction record for the transaction.
	TransactionRecord *services.TransactionRecord `protobuf:"bytes,3,opt,name=transaction_record,json=transactionRecord,proto3" json:"transaction_record,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *TransactionRecordEntry) Reset() {
	*x = TransactionRecordEntry{}
	mi := &file_state_recordcache_recordcache_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TransactionRecordEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TransactionRecordEntry) ProtoMessage() {}

func (x *TransactionRecordEntry) ProtoReflect() protoreflect.Message {
	mi := &file_state_recordcache_recordcache_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TransactionRecordEntry.ProtoReflect.Descriptor instead.
func (*TransactionRecordEntry) Descriptor() ([]byte, []int) {
	return file_state_recordcache_recordcache_proto_rawDescGZIP(), []int{0}
}

func (x *TransactionRecordEntry) GetNodeId() int64 {
	if x != nil {
		return x.NodeId
	}
	return 0
}

func (x *TransactionRecordEntry) GetPayerAccountId() *common.AccountID {
	if x != nil {
		return x.PayerAccountId
	}
	return nil
}

func (x *TransactionRecordEntry) GetTransactionRecord() *services.TransactionRecord {
	if x != nil {
		return x.TransactionRecord
	}
	return nil
}

// *
// As a single transaction is handled a receipt is created. It is stored in state for a configured time
// limit (perhaps, for example, 3 minutes). During this time window, any client can query the node and get the
// receipt for the transaction. The TransactionReceiptEntry is the object stored in state with this information.
type TransactionReceiptEntry struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// *
	// The ID of the node that submitted the transaction to consensus. The ID is the ID of the node as known by the
	// address book. Valid node IDs are in the range 0..2^63-1, inclusive.
	NodeId uint64 `protobuf:"varint,1,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"`
	// *
	// The id of the submitted transaction.
	TransactionId *common.TransactionID `protobuf:"bytes,2,opt,name=transaction_id,json=transactionId,proto3" json:"transaction_id,omitempty"`
	// *
	// The resulting status of handling the transaction.
	Status        services.ResponseCodeEnum `protobuf:"varint,3,opt,name=status,proto3,enum=proto.ResponseCodeEnum" json:"status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TransactionReceiptEntry) Reset() {
	*x = TransactionReceiptEntry{}
	mi := &file_state_recordcache_recordcache_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TransactionReceiptEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TransactionReceiptEntry) ProtoMessage() {}

func (x *TransactionReceiptEntry) ProtoReflect() protoreflect.Message {
	mi := &file_state_recordcache_recordcache_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TransactionReceiptEntry.ProtoReflect.Descriptor instead.
func (*TransactionReceiptEntry) Descriptor() ([]byte, []int) {
	return file_state_recordcache_recordcache_proto_rawDescGZIP(), []int{1}
}

func (x *TransactionReceiptEntry) GetNodeId() uint64 {
	if x != nil {
		return x.NodeId
	}
	return 0
}

func (x *TransactionReceiptEntry) GetTransactionId() *common.TransactionID {
	if x != nil {
		return x.TransactionId
	}
	return nil
}

func (x *TransactionReceiptEntry) GetStatus() services.ResponseCodeEnum {
	if x != nil {
		return x.Status
	}
	return services.ResponseCodeEnum(0)
}

// *
// As transactions are handled and receipts are created, they are stored in state for a configured time
// limit (perhaps, for example, 3 minutes). During this time window, any client can query the node and get the
// receipt for the transaction. The TransactionReceiptEntries is the object stored in state with this information.
// This object contains a list of TransactionReceiptEntry objects.
type TransactionReceiptEntries struct {
	state         protoimpl.MessageState     `protogen:"open.v1"`
	Entries       []*TransactionReceiptEntry `protobuf:"bytes,1,rep,name=entries,proto3" json:"entries,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TransactionReceiptEntries) Reset() {
	*x = TransactionReceiptEntries{}
	mi := &file_state_recordcache_recordcache_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TransactionReceiptEntries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TransactionReceiptEntries) ProtoMessage() {}

func (x *TransactionReceiptEntries) ProtoReflect() protoreflect.Message {
	mi := &file_state_recordcache_recordcache_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TransactionReceiptEntries.ProtoReflect.Descriptor instead.
func (*TransactionReceiptEntries) Descriptor() ([]byte, []int) {
	return file_state_recordcache_recordcache_proto_rawDescGZIP(), []int{2}
}

func (x *TransactionReceiptEntries) GetEntries() []*TransactionReceiptEntry {
	if x != nil {
		return x.Entries
	}
	return nil
}

var File_state_recordcache_recordcache_proto protoreflect.FileDescriptor

const file_state_recordcache_recordcache_proto_rawDesc = "" +
	"\n" +
	"#state/recordcache/recordcache.proto\x12\x05proto\x1a\x11basic_types.proto\x1a\x18transaction_record.proto\x1a\x13response_code.proto\"\xb6\x01\n" +
	"\x16TransactionRecordEntry\x12\x17\n" +
	"\anode_id\x18\x01 \x01(\x03R\x06nodeId\x12:\n" +
	"\x10payer_account_id\x18\x02 \x01(\v2\x10.proto.AccountIDR\x0epayerAccountId\x12G\n" +
	"\x12transaction_record\x18\x03 \x01(\v2\x18.proto.TransactionRecordR\x11transactionRecord\"\xa0\x01\n" +
	"\x17TransactionReceiptEntry\x12\x17\n" +
	"\anode_id\x18\x01 \x01(\x04R\x06nodeId\x12;\n" +
	"\x0etransaction_id\x18\x02 \x01(\v2\x14.proto.TransactionIDR\rtransactionId\x12/\n" +
	"\x06status\x18\x03 \x01(\x0e2\x17.proto.ResponseCodeEnumR\x06status\"U\n" +
	"\x19TransactionReceiptEntries\x128\n" +
	"\aentries\x18\x01 \x03(\v2\x1e.proto.TransactionReceiptEntryR\aentriesBl\n" +
	"\"com.hederahashgraph.api.proto.javaP\x01ZDgithub.com/cordialsys/hedera-protobufs-go/services/state/recordcacheb\x06proto3"

var (
	file_state_recordcache_recordcache_proto_rawDescOnce sync.Once
	file_state_recordcache_recordcache_proto_rawDescData []byte
)

func file_state_recordcache_recordcache_proto_rawDescGZIP() []byte {
	file_state_recordcache_recordcache_proto_rawDescOnce.Do(func() {
		file_state_recordcache_recordcache_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_state_recordcache_recordcache_proto_rawDesc), len(file_state_recordcache_recordcache_proto_rawDesc)))
	})
	return file_state_recordcache_recordcache_proto_rawDescData
}

var file_state_recordcache_recordcache_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
var file_state_recordcache_recordcache_proto_goTypes = []any{
	(*TransactionRecordEntry)(nil),     // 0: proto.TransactionRecordEntry
	(*TransactionReceiptEntry)(nil),    // 1: proto.TransactionReceiptEntry
	(*TransactionReceiptEntries)(nil),  // 2: proto.TransactionReceiptEntries
	(*common.AccountID)(nil),           // 3: proto.AccountID
	(*services.TransactionRecord)(nil), // 4: proto.TransactionRecord
	(*common.TransactionID)(nil),       // 5: proto.TransactionID
	(services.ResponseCodeEnum)(0),     // 6: proto.ResponseCodeEnum
}
var file_state_recordcache_recordcache_proto_depIdxs = []int32{
	3, // 0: proto.TransactionRecordEntry.payer_account_id:type_name -> proto.AccountID
	4, // 1: proto.TransactionRecordEntry.transaction_record:type_name -> proto.TransactionRecord
	5, // 2: proto.TransactionReceiptEntry.transaction_id:type_name -> proto.TransactionID
	6, // 3: proto.TransactionReceiptEntry.status:type_name -> proto.ResponseCodeEnum
	1, // 4: proto.TransactionReceiptEntries.entries:type_name -> proto.TransactionReceiptEntry
	5, // [5:5] is the sub-list for method output_type
	5, // [5:5] is the sub-list for method input_type
	5, // [5:5] is the sub-list for extension type_name
	5, // [5:5] is the sub-list for extension extendee
	0, // [0:5] is the sub-list for field type_name
}

func init() { file_state_recordcache_recordcache_proto_init() }
func file_state_recordcache_recordcache_proto_init() {
	if File_state_recordcache_recordcache_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_state_recordcache_recordcache_proto_rawDesc), len(file_state_recordcache_recordcache_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_state_recordcache_recordcache_proto_goTypes,
		DependencyIndexes: file_state_recordcache_recordcache_proto_depIdxs,
		MessageInfos:      file_state_recordcache_recordcache_proto_msgTypes,
	}.Build()
	File_state_recordcache_recordcache_proto = out.File
	file_state_recordcache_recordcache_proto_goTypes = nil
	file_state_recordcache_recordcache_proto_depIdxs = nil
}
