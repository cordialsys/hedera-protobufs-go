//*
// # hinTS Aggregation Vote Transaction
//
// ### Keywords
// The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT",
// "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this
// document are to be interpreted as described in
// [RFC2119](https://www.ietf.org/rfc/rfc2119) and clarified in
// [RFC8174](https://www.ietf.org/rfc/rfc8174).

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        v5.29.3
// source: auxiliary/hints/hints_preprocessing_vote.proto

package hints

import (
	hints "github.com/cordialsys/hedera-protobufs-go/services/state/hints"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// A transaction body to publish a node's vote for the results of
// aggregating the hinTS keys for a specified construction id. The
// node can either give their explicit aggregation results, or
// delegate their vote to another node's results.
type HintsPreprocessingVoteTransactionBody struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// *
	// The id of the hinTS construction this vote is for. Because
	// the network only ever performs one construction a time, this
	// is somewhat redundant, but included to detect any strange
	// edge cases where a node is so far behind it votes on the wrong
	// construction.
	ConstructionId uint64 `protobuf:"varint,1,opt,name=construction_id,json=constructionId,proto3" json:"construction_id,omitempty"`
	// *
	// The vote the node is casting for the aggregation of the hints
	// in the given construction (which should be ongoing).
	Vote          *hints.PreprocessingVote `protobuf:"bytes,2,opt,name=vote,proto3" json:"vote,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HintsPreprocessingVoteTransactionBody) Reset() {
	*x = HintsPreprocessingVoteTransactionBody{}
	mi := &file_auxiliary_hints_hints_preprocessing_vote_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HintsPreprocessingVoteTransactionBody) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HintsPreprocessingVoteTransactionBody) ProtoMessage() {}

func (x *HintsPreprocessingVoteTransactionBody) ProtoReflect() protoreflect.Message {
	mi := &file_auxiliary_hints_hints_preprocessing_vote_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HintsPreprocessingVoteTransactionBody.ProtoReflect.Descriptor instead.
func (*HintsPreprocessingVoteTransactionBody) Descriptor() ([]byte, []int) {
	return file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescGZIP(), []int{0}
}

func (x *HintsPreprocessingVoteTransactionBody) GetConstructionId() uint64 {
	if x != nil {
		return x.ConstructionId
	}
	return 0
}

func (x *HintsPreprocessingVoteTransactionBody) GetVote() *hints.PreprocessingVote {
	if x != nil {
		return x.Vote
	}
	return nil
}

var File_auxiliary_hints_hints_preprocessing_vote_proto protoreflect.FileDescriptor

const file_auxiliary_hints_hints_preprocessing_vote_proto_rawDesc = "" +
	"\n" +
	".auxiliary/hints/hints_preprocessing_vote.proto\x12(com.hedera.hapi.services.auxiliary.hints\x1a\x1dstate/hints/hints_types.proto\"\x99\x01\n" +
	"%HintsPreprocessingVoteTransactionBody\x12'\n" +
	"\x0fconstruction_id\x18\x01 \x01(\x04R\x0econstructionId\x12G\n" +
	"\x04vote\x18\x02 \x01(\v23.com.hedera.hapi.node.state.hints.PreprocessingVoteR\x04voteBw\n" +
	"/com.hedera.hapi.services.auxiliary.hints.legacyP\x01ZBgithub.com/cordialsys/hedera-protobufs-go/services/auxiliary/hintsb\x06proto3"

var (
	file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescOnce sync.Once
	file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescData []byte
)

func file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescGZIP() []byte {
	file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescOnce.Do(func() {
		file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_auxiliary_hints_hints_preprocessing_vote_proto_rawDesc), len(file_auxiliary_hints_hints_preprocessing_vote_proto_rawDesc)))
	})
	return file_auxiliary_hints_hints_preprocessing_vote_proto_rawDescData
}

var file_auxiliary_hints_hints_preprocessing_vote_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_auxiliary_hints_hints_preprocessing_vote_proto_goTypes = []any{
	(*HintsPreprocessingVoteTransactionBody)(nil), // 0: com.hedera.hapi.services.auxiliary.hints.HintsPreprocessingVoteTransactionBody
	(*hints.PreprocessingVote)(nil),               // 1: com.hedera.hapi.node.state.hints.PreprocessingVote
}
var file_auxiliary_hints_hints_preprocessing_vote_proto_depIdxs = []int32{
	1, // 0: com.hedera.hapi.services.auxiliary.hints.HintsPreprocessingVoteTransactionBody.vote:type_name -> com.hedera.hapi.node.state.hints.PreprocessingVote
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_auxiliary_hints_hints_preprocessing_vote_proto_init() }
func file_auxiliary_hints_hints_preprocessing_vote_proto_init() {
	if File_auxiliary_hints_hints_preprocessing_vote_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_auxiliary_hints_hints_preprocessing_vote_proto_rawDesc), len(file_auxiliary_hints_hints_preprocessing_vote_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_auxiliary_hints_hints_preprocessing_vote_proto_goTypes,
		DependencyIndexes: file_auxiliary_hints_hints_preprocessing_vote_proto_depIdxs,
		MessageInfos:      file_auxiliary_hints_hints_preprocessing_vote_proto_msgTypes,
	}.Build()
	File_auxiliary_hints_hints_preprocessing_vote_proto = out.File
	file_auxiliary_hints_hints_preprocessing_vote_proto_goTypes = nil
	file_auxiliary_hints_hints_preprocessing_vote_proto_depIdxs = nil
}
